...
import { returnProduct } from '../services/supplyApi';
import { getImageUrl } from '../services/api';
import { useKeyboard } from '../context/KeyboardContext';

const ReturnProductModal = ({ product, onClose, onSuccess }) => {
  ...
  const handleReturn = async () => {
    ...
    // Check if return amount doesn't exceed given amount
    // Note: product_count is a string like "450 DONA", need to parse the number
    const countMatch = product.product_count?.match(/\d+/);
    const maxCount = countMatch ? parseInt(countMatch[0]) : 0;

    if (parseFloat(returnAmount) > maxCount) {
      setErrorMessage(t.errorMessage);
      setShowError(true);
      return;
    }
    ...
    try {
      await returnProduct(product.supply_id, parseFloat(returnAmount));
      setShowSuccess(true);
    } catch (err) {
              ...
              <div className="flex items-center mb-4">
                ...
                <div>
                  ...
                  <p className="text-lg text-gray-600">
                    {t.givenAmount}: {product.product_count}
                  </p>
                </div>
              </div>
            </div>
